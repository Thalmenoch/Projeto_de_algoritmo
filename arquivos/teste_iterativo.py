def linha_de_montagem(tempo, troca, entrada, saida):
    estacao = len(tempo[0])  # Aqui é definido o número de estações na linha de montagem
    tempo_total = [[0] * estacao for _ in range(2)]  # Aqui é definido a matriz para armazenar os tempos totais
    caminho_otimo = [[0] * estacao for _ in range(2)]  # Aqui é definido a matriz para rastrear o caminho ótimo

    # Definição dos tempos iniciais
    tempo_total[0][0] = entrada[0] + tempo[0][0]
    tempo_total[1][0] = entrada[1] + tempo[1][0]

    # Preenche]imento da matriz de tempos totais e rastrear o caminho ótimo
